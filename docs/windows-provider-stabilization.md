...
## 10. TerminalView infinite spawn-fail-render loop on Windows (fixed)

**Symptom:** terminal panel fills with repeated red
"Terminals are supported on macOS and Linux." lines, UI pegs and becomes
unusable; conhost count jumps (~32 → ~42) at MonoCode launch and keeps
toggling.

**Root cause:** `src/surfaces/TerminalView.tsx` `applySize()` called
`spawnPty()` whenever `spawned.current` was false, and the `term.onRender`
handler re-invoked `applySize()` on every render. On Windows `pty_spawn`
*always* fails (`src-tauri/src/pty.rs:135`), so: fail → `writeln` red error
→ render → `onRender` → spawn again → fail… an infinite in-process loop per
mounted terminal tab (restored tabs re-arm it on launch). The backend was
never at fault — it errors without spawning anything.

**Fix (no retry, message once):**
- `src/lib/pty.ts`: new `PTY_SUPPORTED = !IS_WINDOWS` (reusing the existing
  `src/lib/platform.ts` flag) + `PTY_UNSUPPORTED_MESSAGE` pinned identical
  to the Rust rejection string.
- `TerminalView.tsx`: `dead` latch, initialized true on Windows; `applySize`
  early-returns when dead (breaks the `onRender` cycle); the unsupported
  notice is written exactly once at mount; keystroke input and the 1s
  meta-poll interval are skipped when dead. Unix retry-on-render behavior is
  untouched.
- Yes — CLIs already had per-OS paths: the Rust backend splits
  `#[cfg(windows)]` vs `#[cfg(unix)]` across resolution, spawning, and
  lifecycle, and the frontend had `IS_MAC`/`IS_WINDOWS`; the terminal view
  was simply the one path that never consulted the flag.
- Tests: `src/lib/pty.test.ts` pins `PTY_SUPPORTED === !IS_WINDOWS` and the
  message string against backend drift. `tsc` clean, 240/240 harness+pty
  tests pass. Rebuilt + reinstalled (`MonoCode_0.1.29_x64-setup.exe`,
  silent `/S` to `E:\Installed\MonoCode`).

**Follow-up: repeated single lines from remounts.** The latch kills the
infinite loop, but each true remount (tab switches, layout restores create a
fresh component instance with fresh refs) reprinted the notice once — N
visits, N lines in the same panel. Hardened with a module-level
`markUnsupportedNotified(id)` in `src/lib/pty.ts` (window-scoped,
one notice per terminal id per launch), wired into the mount effect, with a
unit test (`pty.test.ts`: first call true, repeats false, per-id
independent). Unix behavior untouched.

**Install-verification lesson (same day):** a SHA256 mismatch between
`E:\Installed\MonoCode\monocode.exe` and `target\release\monocode.exe`
caused a false "stale install" alarm — the bytes differ only because Tauri
patches bundle-type metadata into the binary *after* bundling
(`Patching ... with bundle type information: nsis` in the build log).
Reliable check instead: fingerprint the current `dist` asset hash
(`dist/index.html` → `assets/index-<hash>.js`) and count it inside each
binary — both carried the current hash exactly once, proving the installed
build contains the fix. Also learned the hard way: PowerShell
`Get-ChildItem -Include` without `-Recurse` silently matches nothing, and
`[Text.Encoding]::Latin1` does not exist in Windows PowerShell 5.1 (use
`GetEncoding("iso-8859-1")`) — both produced false negatives during this
diagnosis.

**On the conhost-count jump:** hidden is not conhost-free on Windows —
`CREATE_NO_WINDOW` hides the window, but short-lived console processes
(probes, catalog `models`/`agent list`, serve children) still cycle transient
conhost objects, so a launch-time wave that drains is expected. *Persistently
visible, focus-stealing* windows are the T3/spike/MCP processes in §8, not
these.

---

## 11. Unhidden git/gh spawns across the backend (fixed)

**Symptom (user's Task Manager screenshot):** dozens of `gh.exe` + `git.exe`
processes while MonoCode runs; visible consoles piling to "5+ open all the
time," stealing focus until the screen is unusable. Cascade screenshots
confirmed ~10 deep stacks of console windows.

**Root cause:** the `CREATE_NO_WINDOW` audit from the provider work had
covered only `harness.rs`. The rest of the backend spawned consoles bare:
`fs.rs` (`git_ls_files`, `git_hash_object`, `gh_run`, `git_checked`,
`git_output`, `git_branch_name`, `git_stdout`, `git clone`), `search.rs`
(`git_grep` — per-keystroke file search), all with no flag. Triggers are
hot: `GitChangesPanel` repolls every 2s (`GIT_POLL_MS = 2000`) plus on
focus/visibility/git-events, explorer lists per navigation, search per
keystroke — in big, busy repos each call is slow, windows linger and pile.
(`checkpoint.rs`/`session_store.rs` git sites are test-only;
`open`/`explorer`/`xdg-open`, macOS `security`, unix `ps`/`sh` intentionally
untouched.)

**Fix:** one shared `pub(crate) hide_console_window()` in
`src-tauri/src/harness.rs` (sets `CREATE_NO_WINDOW` on Windows, no-op
elsewhere; `isolate_child` now routes through it), called at all 9
production sites. `cargo fmt --check` clean, `cargo check` clean, full suite
shows only the 13 pre-existing CRLF failures. Rebuilt + reinstalled with
the app fully closed (NSIS silently skips a locked exe while reporting
success — kill `monocode.exe` first, then verify via the dist asset-hash
fingerprint, not SHA256, since Tauri patches bundle metadata post-build).

**Answer to "do CLIs already have per-OS flags":** yes for everything the
provider project touched (`harness.rs` resolver/spawn/lifecycle), no for the
older `fs`/`search` helpers — this fix closes that gap. One rule now covers
the whole backend: no `Command::new` without `hide_console_window` (or
`isolate_child`) unless the window is intentional.
