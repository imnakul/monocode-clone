...
async function listAllModels(rpc: JsonRpcClient): Promise<AgentModel[]> {
  ...
  do {
    ...
    const page = codexModelPage(response);
    rows.push(...page.rows);
...
/**
 * Accept both spellings of the `model/list` envelope: app-server builds have
 * shipped `nextCursor` and `next_cursor`, with rows under `data` or `models`.
 * A wrong key silently truncates the catalog at page one - and the newest
 * models live past it.
 */
export function codexModelPage(response: unknown): {
  rows: unknown[];
  nextCursor: string | null;
} {
  const rec = asRecord(response);
  const rowsRaw = rec?.data ?? rec?.models;
  const rows = Array.isArray(rowsRaw) ? rowsRaw : [];
  const cursor =
    stringField(rec, "nextCursor") ?? stringField(rec, "next_cursor");
  return {
    rows,
    nextCursor: cursor && cursor.trim() !== "" ? cursor : null,
  };
}
