...
describe("codexModelPage", () => {
  it("reads rows and the paging cursor from either app-server spelling", () => {
    expect(
      codexModelPage({ data: [{ model: "gpt-5.6-luna" }], nextCursor: "c1" }),
    ).toEqual({ rows: [{ model: "gpt-5.6-luna" }], nextCursor: "c1" });
    expect(
      codexModelPage({
        models: [{ model: "gpt-5.6-terra" }],
        next_cursor: "c2",
      }),
    ).toEqual({ rows: [{ model: "gpt-5.6-terra" }], nextCursor: "c2" });
  });

  it("treats a missing or blank cursor as the last page", () => {
    expect(codexModelPage({ data: [] })).toEqual({
      rows: [],
      nextCursor: null,
    });
    expect(codexModelPage({ data: [], nextCursor: "" })).toEqual({
      rows: [],
      nextCursor: null,
    });
    expect(codexModelPage(null)).toEqual({ rows: [], nextCursor: null });
  });
});
