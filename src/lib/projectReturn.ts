...
export function reconcileProjectReturn({
  ...
  if (
    next.size === safeMemory.size &&
    [...next].every(([project, id]) => safeMemory.get(project) === id)
  )
    return safeMemory;
  return next;
}
