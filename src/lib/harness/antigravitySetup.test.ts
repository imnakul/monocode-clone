...
describe("Antigravity explicit sign-in", () => {
  ...
  // A browser that never opens must end the bounded sign-in flow, not hang.
  it("reports browser launch failure and cleans up", async (): Promise<void> => {
    boundary.open.mockRejectedValueOnce(new Error("open blocked"));
    await expect(signInAntigravity()).rejects.toThrow("Could not open the Google sign-in page");
    expect(boundary.kill).toHaveBeenCalled();
  });
});
