...
async function handshake(resume = false): Promise<void> {
...
afterEach(async (): Promise<void> => { await agy.forgetAntigravitySession(input.sessionId); boundary.writes.length = 0; events.length = 0; vi.clearAllMocks(); });
describe("official Antigravity ACP child boundary", (): void => {
  ...
  it("routes string-ID approvals and native reasoning/tool updates without losing the option ID", async (): Promise<void> => {
    const turn = agy.sendAntigravityTurn(input); await handshake(); const prompt = await request("session/prompt");
  ...
  it("selects the advertised model and permission mode via config options", async (): Promise<void> => {
    const turn = agy.sendAntigravityTurn({ ...input, model: "antigravity:claude", runtimeMode: "auto-accept-edits" }); await handshake();
  ...
  it("rejects legacy IDs before spawning and never creates a replacement after failed resume", async (): Promise<void> => {
    ...
    await failed; expect(boundary.writes.some((m) => m.method === "session/new")).toBe(false); expect(boundary.kill).toHaveBeenCalled();
  });
  it("surfaces rate limits once without retrying a prompt", async (): Promise<void> => {
    const turn = agy.sendAntigravityTurn(input); const failed = expect(turn).rejects.toThrow("Quota exhausted"); await handshake();
  ...
  it("rejects unsupported attachments before sending a text-only substitute", async (): Promise<void> => {
    ...
    const failed = expect(turn).rejects.toThrow(/does not support/); await handshake(); await failed;
    expect(boundary.writes.some((m) => m.method === "session/prompt")).toBe(false);
  });
  // Catches accidentally retaining CLI flags, respawning per turn, or using session/load.
  it("keeps two turns on one ACP process and resumes the tagged session after parking", async (): Promise<void> => {
    ...
    await handshake(); const p1 = await request("session/prompt"); reply(p1.id, { stopReason: "end_turn" }); await first;
    expect(boundary.spawn).toHaveBeenCalledWith("test-agy", "/fake/agy_acp_server", [], "/workspace");
    ...
    const third = agy.sendAntigravityTurn(input); await handshake(true);
  ...
  it("reports process death and resumes the same native session on the next turn", async (): Promise<void> => {
    const first = agy.sendAntigravityTurn(input); await handshake(); const p1 = await request("session/prompt"); reply(p1.id, { stopReason: "end_turn" }); await first;
    boundary.exit(3);
