...
describe("ComposerAction", () => {
  // Intentional divergence from upstream: while an agent is busy, the send
  // action keeps its queue/steer-aware label and Stop stays visible. See
  // docs/LOCAL-CHANGELOG.md (0.1.51-local1-upstream-import, "Composer send
  // action") for the decision record.
  it("keeps Stop visible and labels send with the follow-up action", () => {
    const empty = renderAction(true, false);
    expect(empty).toContain('aria-label="Stop"');
    expect(empty).not.toContain("composer-send");
