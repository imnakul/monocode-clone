...
import { contextTooltip } from "../model/contextUsage";
...
describe("ContextMeter", () => {
  ...
  it("renders circular gauge with accessible title when usage is given", () => {
    ...
    expect(markup).toContain(
      `aria-label="Context window: ${(72_300).toLocaleString()} / ${(200_000).toLocaleString()} tokens (36%)"`,
    );
  ...
  it("renders Codex gauge and accessible aria-label", () => {
    ...
    expect(markup).toContain(
      `aria-label="Context window: ${(50_000).toLocaleString()} / ${(128_000).toLocaleString()} tokens (39%)"`,
    );
  });
  ...
  it("renders gauge and ring under default preferences", () => {
    ...
    expect(markup).toContain(
      `aria-label="Context window: ${(58_400).toLocaleString()} / ${(100_000).toLocaleString()} tokens (58%)"`,
    );
    expect(markup).toContain("<svg");
    expect(markup).toContain("<circle");
  });
